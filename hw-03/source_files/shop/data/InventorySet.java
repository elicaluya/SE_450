...
final class InventorySet implements Inventory {
  ...
  Record checkOut(Video video) {
    // TODO  
	  if (video == null) throw new IllegalArgumentException("Null video value for check out");
	  RecordObj r = (RecordObj)_data.get(video);
  ...
  Record checkIn(Video video) {
    // TODO  
	  if (video == null) throw new IllegalArgumentException("Null video value for check in");
	  RecordObj r = (RecordObj)_data.get(video);
	  // If the video requested has no Record, throw IllegalArgumentException
	  if (r == null || video == null)
  ...
  Map clear() {
    // TODO  
	  Map<Video,Record> _dataCopy = _data;
	  _data = new HashMap<Video,Record>();
	  return _dataCopy;
  }
